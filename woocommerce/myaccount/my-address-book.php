...
?>
<?php
// Display on the add new shipping address page.
if ( $type === 'shipping' ) {
	...
	if ( $wc_address_book->get_wcab_option( 'shipping_enable' ) === true ) {
		...
		// Hide the shipping address book if there are no addresses to show and no ability to add new ones.
		...
		if ( ! empty( $woo_address_book_shipping_address ) && ! $hide_shipping_address_book ) {
			...
			<div class="address_book shipping_address_book" data-addresses='<?php echo esc_attr( $count_section ); ?>' data-limit='<?php echo esc_attr( $save_limit ); ?>'>
				<h2><?php esc_html_e( 'Shipping Addresses On File', 'woo-address-book' ); ?></h2>
<!-- 				<header> -->
...
<!-- 				</header> -->
				...
				foreach ( $woo_address_book_shipping_address_book as $woo_address_book_name => $woo_address_book_fields ) {
					...
					if ( $woo_address_book_formatted_address ) {
						?>
						<li class="wc-address-book-address">
							<div class="wc-address-book-meta">
							...
							<div class="summit-address-title">
								<a href="<?php echo esc_url( $wc_address_book->get_address_book_endpoint_url( $woo_address_book_name, 'shipping' ) ); ?>" class="wc-address-book-edit"><?php echo wp_kses( $woo_address_book_formatted_address, array( 'br' => array() ) ); ?></a>
							</div>
						</li>
						<?php
					}
