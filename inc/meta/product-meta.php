...
/**
 * Insert product video as second item in the product gallery
 */
function summit_insert_video_in_gallery() {
	global $product;
	...
	$video_url = summit_get_product_video( $product_id );
	...
	$poster_thumb = $poster_id ? wp_get_attachment_image_url( $poster_id, 'woocommerce_thumbnail' ) : '';
	
	// Output video slide - thumbnail shows poster with play icon, main slide shows video
	echo '<div class="woocommerce-product-gallery__image summit-product-video-slide" data-thumb="' . esc_url( $poster_thumb ) . '">';
	echo '<div class="summit-product-video-wrapper">';
	echo '<video src="' . esc_url( $video_url ) . '" controls playsinline preload="metadata"></video>';
	echo '</div>';
	echo '</div>';
}
add_action( 'woocommerce_product_thumbnails', 'summit_insert_video_in_gallery', 5 );

/**
 * Enqueue video gallery script on frontend
 */
function summit_enqueue_product_video_gallery_script() {
	if ( is_product() ) {
		wp_enqueue_script(
			'summit-product-video-gallery',
			get_template_directory_uri() . '/js/summit-product-video-gallery.js',
			array( 'jquery' ),
			'1.0.0',
			true
		);
...
add_action( 'wp_enqueue_scripts', 'summit_enqueue_product_video_gallery_script' );
